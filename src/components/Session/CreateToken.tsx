...
export default function CreateToken() {
    ...
    const createRequestToken = async ()=>{
        const res = await fetch(
          ...
          if (finalRes.success) {
            ...
            setIsAuth(true)
            open(`https://www.themoviedb.org/authenticate/${finalRes.request_token}?redirect_to=http://localhost:5173/session`,'_self')
          }
    ...
    const askForPermission = (request_token:string)=>{
        open(`https://www.themoviedb.org/authenticate/${request_token}?redirect_to=http://localhost:5173/session`,'_self')
    }
  return (
    <div className="max-w-[1440px] mx-auto mt-72 md:mt-0 bg-white p-10  rounded-[20px] shadow-lg">
        ...
        {!isAuth&&localStorage.getItem('token')!=url_request_token&&<button onClick={createRequestToken} className="bg-[#ff5300] hover:bg-[#ff5500e2] w-fit px-20 py-3 rounded-md text-[22px] font-bold  my-10">Create</button>}
